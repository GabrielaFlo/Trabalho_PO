...
int partition(vector<int>& arr, int low, int high, long long& comparisons, long long& swaps) {
    ...
    // Alteração aqui: j agora vai até high - 1, pois o pivô já está no índice high
    for (int j = low; j <= high - 1; j++) {  // Corrigido para j <= high - 1
        comparisons++;
        if (arr[j] >= pivot) {  // Modificação para ordem decrescente
            i++;
            swap(arr[i], arr[j]);
            swaps++;
        }
...
int main() {
    const int SIZE = 100000;  // Se o tamanho for muito grande, considere reduzir para 10000
    vector<int> arr(SIZE);
    ...
    // Calcula o tempo de execução do Quick Sort Iterativo
